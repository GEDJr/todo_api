...
use surrealdb::{
    ...
    kvs::Datastore, dbs::Response, dbs::Session,
};
...
impl DB {
    pub async fn execute(
        ...
        let res: Vec<Response> = self.ds.execute(query, &self.sesh, vars).await?;
        Ok(res)
    }

    pub async fn add_task(&self, title: String) -> Result<Object, crate::error::Error> {
        let sql: &str = "CREATE tasks SET title = $title, completed = false";
        let vars: BTreeMap<String, Value> = map!["title".into() => Value::Strand(title.into())];
        let res: Vec<Response> = self.execute(sql, Some(vars)).await?;
    ...
    pub async fn get_task(&self, id: String) -> Result<Object, crate::error::Error> {
        ...
        let tid: String = format!("{}", id);
        let vars: BTreeMap<String, Value> = map!["th".into() => thing(&tid)?.into()];
        let ress: Vec<Response> = self.execute(sql, Some(vars)).await?;
